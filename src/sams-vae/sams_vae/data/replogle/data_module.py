...
class ReplogleDataModule(PerturbationDataModule):
	...
	##VJ hard-coding
        self.adata = anndata.read_h5ad('/lustre/scratch126/cellgen/team298/vm11/LEARN/scEvalsJam/sams-vae/1gene-replogle-essential-split_test1_PREPROCESSED_INTEGERS.h5ad')

        # define splits
        train_idx = self.adata.obs[self.adata.obs["split"] == "train"].index
        val_idx = self.adata.obs[self.adata.obs["split"] == "val"].index
        test_idx = self.adata.obs[self.adata.obs["split"] == "test"].index
        ...
        # vj
        x_tr_mean = X_tr.type(torch.float).mean(0)
        x_tr_std = X_tr.type(torch.float).std(0)
