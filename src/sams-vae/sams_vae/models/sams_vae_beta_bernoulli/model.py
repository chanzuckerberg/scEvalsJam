...
class SAMSVAEBetaBernoulliModel(torch.nn.Module):
    def __init__(
        self,
        n_latent: int,
        n_treatments: int,
        n_phenos: int,
        mask_prior_alpha: float,
        mask_prior_beta: float,
        ...
        self.register_buffer(
            "p_mask_probs_alpha",
            mask_prior_alpha * torch.ones((n_treatments, n_latent)),
        )

        self.register_buffer(
            "p_mask_probs_beta", mask_prior_beta * torch.ones((n_treatments, n_latent))
        )
    ...
    def get_var_keys(self) -> List[str]:
        return ["z_basal", "E", "mask_probs", "mask"]
    ...
    def forward(
        ...
        generative_dists["p_mask_probs"] = Beta(
            self.p_mask_probs_alpha, self.p_mask_probs_beta
        )

        if condition_values.get("mask_probs") is not None:
            mask_probs = condition_values["mask_probs"]
            if len(mask_probs.shape) == 2:
                mask_probs = mask_probs.unsqueeze(0).expand((n_particles, -1, -1))
            samples["mask_probs"] = mask_probs
        else:
            samples["mask_probs"] = generative_dists["p_mask_probs"].sample(
                (n_particles,)
            )

        generative_dists["p_mask"] = Bernoulli(
            logits=torch.logit(samples["mask_probs"])
        )
