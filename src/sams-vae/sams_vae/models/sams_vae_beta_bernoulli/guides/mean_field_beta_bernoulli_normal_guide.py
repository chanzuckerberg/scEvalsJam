...
class SAMSVAEBetaBernoulliMeanFieldBetaBernoulliNormalGuide(nn.Module):
    def __init__(
        self,
        n_latent: int,
        n_treatments: int,
        ...
        mask_probs_init_alpha: float = 1,
        mask_probs_init_beta: float = 1,
        ...
        self.param_dict["q_mask_probs_alpha"] = torch.nn.Parameter(
            mask_probs_init_alpha * torch.ones((n_treatments, n_latent))
        )
        self.param_dict["q_mask_probs_beta"] = torch.nn.Parameter(
            mask_probs_init_beta * torch.ones((n_treatments, n_latent))
        )
    ...
    def get_var_keys(self):
        var_keys = ["z_basal", "E", "mask_probs", "mask"]
        return var_keys
    ...
    def forward(
        ...
        """
        Compute
        q(z_basal, \alpha, M, E | X, D) = q(\alpha) q(M | \alpha) q(E) q(z_basal | M, E, X, D)
        and sample
        ...
        guide_distributions["q_mask_probs"] = Beta(
            self.param_dict["q_mask_probs_alpha"],
            self.param_dict["q_mask_probs_beta"],
        )

        guide_samples["mask_probs"] = guide_distributions["q_mask_probs"].rsample(
            (n_particles,)
        )

        guide_distributions["q_mask"] = GumbelSoftmaxBernoulliStraightThrough(
            temperature=self.gs_temperature,
            logits=torch.logit(guide_samples["mask_probs"]),
        )

        # already has n_particles from sampling mask_probs
