...
class SAMSVAEBetaBernoulliCorrelatedBernoulliNormalGuide(nn.Module):
    def __init__(
        self,
        n_latent: int,
        n_treatments: int,
    ...
    def forward(
        ...
        """
        Compute
        q(z_basal, \alpha, M, E | X, D) = q(\alpha) q(M | \alpha) q(E|M) q(z_basal | M, E, X, D)
        and sample
        ...
        # q(\alpha) q(M | \alpha)
        guide_distributions["q_mask_probs"] = Delta(
            torch.sigmoid(self.param_dict["q_mask_logits"])
        )
