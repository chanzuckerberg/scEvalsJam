...
def get_de_genes(anndata_x: ad.AnnData, anndata_y: ad.AnnData, method: str, top_k: int) -> dict:
        """
        ...
        ## -- Extract DE genes
        up_genes = de_df[de_df["logfoldchanges"]>0]["names"][1:top_k]
        dn_genes = de_df[de_df["logfoldchanges"]<0]["names"][1:top_k]
        
        return {"de_up":np.asarray(up_genes),"de_dn":np.asarray(dn_genes)}
